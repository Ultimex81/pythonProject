...
Ui_HDF_Browser_Window, Q_HDF_Browser_Window = loadUiType('BAMline_H5_Browser.ui')  # connect to the GUI for the program
...
class HDF_Browser(Ui_HDF_Browser_Window, Q_HDF_Browser_Window):
    ...
    def set_path(self):
        ...
        #     return keys
        #
        # self.structure = get_dataset_keys(self.f)
        # self.test = []
        # self.treeView.setColumnCount(2)
        #
        # for item in self.structure:
        #     itemparts = item.split('/')
        #     entry = QTreeWidgetItem([item,itemparts[0]])
        #     item.addChild(entry)
        #
        # self.test.append(entry)
        #
        # self.treeView.insertTopLevelItems(0, self.test)
